...
async function runTests() {
  ...
  try {
    ...
    assert(
      ...
      'GET /api/health returns status 200 and healthy'
    );

    // ─── 2. Create a fresh test user so we have a reliable auth token ────────
    const randomSuffix = Math.floor(Math.random() * 1_000_000);
    const testEmail = `testuser${randomSuffix}@globetrotter.com`;
    ...
    assert(
      signupRes.status === 201 && signupData.success === true && signupData.data?.user.email === testEmail,
      'POST /api/auth/signup creates user with 10-digit phone and returns JWT'
    );

    // ─── 3. Login with the freshly created user ──────────────────────────────
    ...
    assert(
      loginRes.status === 200 && loginData.success === true && !!loginData.data?.token,
      'POST /api/auth/login valid credentials returns token'
    );
    ...
    assert(
      ...
      'POST /api/auth/signup rejects weak password (<8 chars / no symbols)'
    );

    // ─── 6. Reject Short Phone (<10 digits) ──────────────────────────────────
    ...
    assert(
      shortPhoneRes.status === 400 && shortPhoneData.success === false,
      'POST /api/auth/signup rejects phone number under 10 digits'
    );

    // ─── 7. Google OAuth — New User Auto-Provisioning ────────────────────────
    ...
    assert(
      googleRes.status === 200 && googleData.success === true && googleData.data?.user.email === googleEmail,
      'POST /api/auth/google provisions new user and returns JWT'
    );

    // ─── 8. Google OAuth — Existing User Handshake ───────────────────────────
    const googleExistingRes = await fetch(`${BASE_URL}/auth/google`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: googleEmail, first_name: 'Google', last_name: 'Explorer' }),
    });
    const googleExistingData = (await googleExistingRes.json()) as ApiResponse<{ user: any; token: string }>;
    assert(
      googleExistingRes.status === 200 && googleExistingData.success === true && !!googleExistingData.data?.token,
      'POST /api/auth/google logs in existing user seamlessly'
    );
    ...
    assert(
      meRes.status === 200 && meData.data?.email === testEmail,
      'GET /api/users/me with valid Bearer token returns profile',
      `status=${meRes.status} email=${meData.data?.email}`
    );

    // ─── 10. GET /api/users/me without Token → 401 ───────────────────────────
    ...
    const updateRes = await fetch(`${BASE_URL}/users/me`, {
      ...
      body: JSON.stringify({ bio: 'Updated travel bio with verified validation.' }),
    });
    const updateData = (await updateRes.json()) as ApiResponse<any>;
    assert(
      updateRes.status === 200 && updateData.data?.bio === 'Updated travel bio with verified validation.',
      'PUT /api/users/me updates profile successfully',
      `status=${updateRes.status} bio="${updateData.data?.bio}"`
    );
    ...
    assert(
      citiesRes.status === 200 && Array.isArray(citiesData.data) && citiesData.data.length >= 10,
      'GET /api/cities returns full city list'
    );

    // ─── 13. GET /api/cities?search=Paris ────────────────────────────────────
    ...
    assert(
      searchCityRes.status === 200 &&
        Boolean(searchCityData.data && searchCityData.data.length > 0 && searchCityData.data[0].name === 'Paris'),
      'GET /api/cities?search=Paris filters correctly'
    );

    // ─── 14. GET /api/activities?category=food ───────────────────────────────
    const catRes = await fetch(`${BASE_URL}/activities?category=food`);
    const catData = (await catRes.json()) as ApiResponse<any[]>;
    assert(
      catRes.status === 200 &&
        Boolean(catData.data && catData.data.every((a: any) => a.category.toLowerCase() === 'food')),
      'GET /api/activities?category=food filters activities correctly'
    );

    console.log(`\n🏁 Full Integration Test Summary: ${passed} passed, ${failed} failed.\n`);
  } catch (err) {
